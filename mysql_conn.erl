...
%% return: [#ok_packet{}|_] | [#error_packet{}] 
do_query(Sock, RecvPid, LogFun, Query, Version) ->
	...
	ok ->
	    get_query_response(LogFun,RecvPid,
				    Version, ?SERVER_MORE_RESULTS_EXIST);
	{error, Reason} ->
	    Msg = io_lib:format("Failed sending data "
				"on socket : ~p",
				[Reason]),
	    [#error_packet{error=Reason}]
    end.
...
%% return: {ok, [#ok_packet{} | _], NewState} | [#ok_packet{}|_] |  [#error_packet{}] 
...
prepare_and_exec(State, Name, Version, Stmt, Params) ->
	...
	[#error_packet{}] = Err ->
	    Err;
	Other ->
	    [#error_packet{error={unexpected_msg, Other}}]
    end.
...
%% Returns :  [ #error_packet{}]  or [#ok_packet{}|_]
%%--------------------------------------------------------------------
get_query_response(LogFun, RecvPid, Version, ServerStatus) ->
	case catch get_query_response2(LogFun, RecvPid, Version, ServerStatus) of
		{error, Result} ->
			Result;
		Result ->
			Result
...
get_query_response2(_LogFun, _RecvPid, _Version, ?SERVER_NO_STATUS) ->
	[];
get_query_response2(LogFun, RecvPid, Version, ?SERVER_MORE_RESULTS_EXIST) ->
    case do_recv(LogFun, RecvPid, undefined) of
	{ok, <<0:8, Rest/binary>>, _} ->
		    %% No Tabular data
		    {AffectedRows, Rest1} = mysql_util:length_coded_binary(Rest),
			{_InsertId, Rest2} = mysql_util:length_coded_binary(Rest1),
			<<ServerStatus:16/little, _WarningCount:16/little, _Msg/binary>> = Rest2,
		    [#ok_packet{affected_rows=AffectedRows}|get_query_response2(LogFun, RecvPid, Version, ServerStatus band ?SERVER_MORE_RESULTS_EXIST)];
	{ok, <<?RESP_EOF:8>>, _} ->
			[];
	{ok, <<?RESP_EOF:8, _WarningCount:16/little, ServerStatus:16/little>>, _} ->
			get_query_response2(LogFun, RecvPid, Version, ServerStatus band ?SERVER_MORE_RESULTS_EXIST);
	{ok, <<?RESP_ERROR:8, _Code:16/little, Message/binary>>, _} ->
		    throw({error,[#error_packet{error=Message}]});
	{ok, _Data, _} ->
		    %% Tabular data received
		    case get_fields(LogFun, RecvPid, [], Version) of
			{ok, Fields} ->
				case get_rows(Fields, LogFun, RecvPid, []) of
					{ok, Rows, ServerStatus} ->
						[#result_packet{%fieldinfo=Fields,
										rows=Rows}|
										   get_query_response2(LogFun, RecvPid, Version, ServerStatus band ?SERVER_MORE_RESULTS_EXIST)];
					{error, Reason} ->
						throw({error, [#error_packet{error=Reason}]})
				end;
			{error, Reason} ->
			    throw({error, [#error_packet{error=Reason}]})
		    end;
	...
	{ok, Packet, _Num} ->
		case Packet of
			<<?RESP_EOF, _WarningCount:16/little, ServerStatus:16/little>> ->
				{ok, lists:reverse(Res), ServerStatus};
			<<?RESP_EOF, _Rest/binary>>  ->
				{ok, lists:reverse(Res), ?SERVER_NO_STATUS};
			_ ->
